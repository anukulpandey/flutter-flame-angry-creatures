...
class MyGame extends Forge2DGame with HasTappables {
  ...
  FutureOr<void> onLoad() async {
    await super.onLoad();
    Vector2 gameSize = screenToWorld(camera.viewport
        .effectiveSize); //now gamesize is converted to coordinate system for forge2d game
    add(SpriteComponent()
    ...
    Shape shape = EdgeShape()
      ..set(
          ...
          Vector2(
              gameSize.x,
              gameSize.y *
                  0.90)); //you can play with this by changing the coordinates of vectors it will form different lines
...
class Player extends BodyComponent with Tappable {
  ...
  Future<void> onLoad() async {
    add(SpriteComponent()
      ..size = Vector2.all(6)
      ..sprite = await gameRef.loadSprite('creature.gif')
  ...
  Body createBody() {
    ...
    BodyDef bodyDef = BodyDef(
        ...
        type: BodyType.dynamic); //this can interact with others
    FixtureDef fixtureDef = FixtureDef(shape,
        friction: 0.4, density: 1); // determines physical properties
    return world.createBody(bodyDef)..createFixture(fixtureDef);
  }

  @override
  bool onTapDown(TapDownInfo info) {
    body.applyLinearImpulse(Vector2(10, 0) * 100);
    return false;
  }
}
